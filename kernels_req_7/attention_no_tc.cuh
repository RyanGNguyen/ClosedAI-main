...
__global__ void flash_attn_kernel_no_tc(
    const float *Q,
    const float *K,
    const float *V,
    float *O,
...
void attention_forward_no_tc(float *out, float *qkvr, float *att, float *inp, int B, int T, int C, int NH) {
    ...
    size_t smem_size = (Q_TILE_SIZE * HS + KV_TILE_SIZE * HS + KV_TILE_SIZE * HS +
                        Q_TILE_SIZE * KV_TILE_SIZE + Q_TILE_SIZE * HS) * sizeof(float);
    flash_attn_kernel_no_tc<<<num_blocks, num_threads, smem_size>>>(q, k, v, attention_out, B, NH, T, HS, scale);
    cudaDeviceSynchronize();
