...
__global__ void local_flash_attn_kernel_tf32(
    ...
    // For a tile of queries [q_tile_offset, q_tile_offset + Q_TILE_SIZE),
    // the union of all windows is [max(0, q_tile_offset - WINDOW_SIZE), q_tile_offset + Q_TILE_SIZE)
    int window_start = max(0, q_tile_offset - WINDOW_SIZE);
    int window_end = q_tile_offset + Q_TILE_SIZE;
    int start_tile = window_start / KV_TILE_SIZE;
    int end_tile = CEIL_DIV(window_end, KV_TILE_SIZE);

    for (int tile = start_tile; tile < end_tile; tile++) {
        for (int kv_idx = 0; kv_idx < KV_TILE_SIZE; kv_idx++) {
            int kv_offset = tile * KV_TILE_SIZE + kv_idx;
            if (kv_offset < T && kv_offset >= window_start && kv_offset < window_end) {
        ...
        // Apply softmax scaling, causal masking, and per-query window masking
        for (int qk_idx = tid; qk_idx < KV_TILE_SIZE * Q_TILE_SIZE; qk_idx += blockDim.x) {
            int kv_idx = qk_idx % KV_TILE_SIZE;
            int q_idx = qk_idx / KV_TILE_SIZE;
            int q_offset = q_tile_offset + q_idx;
            ...
            // For each query, the window is [max(0, q_offset - WINDOW_SIZE), q_offset]
            int query_window_start = max(0, q_offset - WINDOW_SIZE);

            if (q_offset >= T || kv_offset >= T || kv_offset > q_offset || kv_offset < query_window_start) {
...
void  attention_forward(float *out, float *qkvr, float *att, float *inp, int B, int T, int C, int NH){
    ...
    local_flash_attn_kernel_tf32<<<num_blocks, num_threads, smem_size>>>(q, k, v, attention_out, B, NH, T, HS, scale);
    cudaDeviceSynchronize();
