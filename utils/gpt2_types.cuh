...
#ifndef GPT2_TYPES_CUH
#define GPT2_TYPES_CUH
...
typedef struct {
    ...
    float* output;
    // Saliency arrays 
    // float* saliency_ln1;      // (L * C)   : for ln1 -> QKV projection (C per layer)
    // float* saliency_atty;     // (L * C)   : for attention output projection (C per layer)
    // float* saliency_ln2;      // (L * C)   : for ln2 -> FC1 (C per layer)
    // float* saliency_fch_gelu; // (L * 4*C) : for FC2 (4C per layer)
} ActivationTensors;
...
typedef struct {
    ...
    float mean_loss;
    float* cpu_losses;
    // AWQ calibration support
    int calibration_enabled;  // Flag to enable/disable calibration accumulation
    float* saliency_memory;   // (L*C*7) - accumulated channel magnitudes for AWQ
} GPT2;

#define GPT2_EOT 50256

#endif // GPT2_TYPES_CUH
